...
serve(async (req) => {
  ...
  try {
    ...
    const canProceed = await checkRateLimit(supabaseClient, user.id, 'fetch_highergov', 5, 24);
    if (!canProceed) {
      console.log(`Rate limit exceeded for user ${user.id}`);
      return new Response(JSON.stringify({ 
        error: 'Rate limit exceeded. You can sync HigherGov up to 5 times per day.' 
      }), {
        status: 429,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
...
"EMS 911" (not "911"), "Non-Emergency", "IFT", "BLS", "ALS", "CCT", "MEDEVAC", "Billing", "CQI", "EMS Tech", "VR/Sim", "Call Center", "LA28 Olympics", "Paralympics", "FIFA World Cup", "Soccer/Football"
      ...
      try {
        const { error: insertError } = await supabaseClient
          ...
          .insert({
            ...
            estimated_value_min: opp.val_est_low ? parseFloat(opp.val_est_low) : null,
            estimated_value_max: opp.val_est_high ? parseFloat(opp.val_est_high) : null,
            issue_date: opp.posted_date || null,
            questions_due: null,
            pre_bid_meeting: null,
            proposal_due: opp.due_date || new Date().toISOString(),
            term_length: null,
